# Anonymity Networks with BRT

Currently only Tor and I2P have been integrated into BRT. The usage of
...
is made or until `BRTd` is shutdown and restarted with only public
connections enabled.

Anonymity networks can also be used with `BRT-wallet-cli` and
`BRT-wallet-rpc` - the wallets will connect to a daemon through a proxy. The
...
`--tx-proxy` which tells `BRTd` the ip/port of a socks proxy provided by a
...
which tells `BRTd` that ".onion" p2p addresses can be forwarded to a socks
...
`--anonymous-inbound`. This option tells `BRTd` the inbound address, network
...
which tells `BRTd` that a max of 25 inbound Tor connections are being
received at address "rveahdfho7wo4b2m.onion:28083" and forwarded to `BRTd`
localhost port 28083, and a default max I2P connections are being received at
address "cmeua5767mz2q5jsaelk2rxhf67agrwuetaso5dzbenyzwlbkg2q.b32.i2p:5000" and
forwarded to `BRTd` localhost port 30000.
...
`BRTd` RPC port - which is independent from the configuration for incoming
...
```
HiddenServiceDir /var/lib/tor/data/BRT
...
This will store key information in `/var/lib/tor/data/BRT` and will forward
"Tor port" 28083 to port 28083 of ip 127.0.0.1. The file
`/usr/lib/tor/data/BRT/hostname` will contain the ".onion" address for use
...
system clock is fairly accurate - many peers on the BRT network should have
similar timestamps.
...
### Intermittent BRT Syncing

If a user only runs `BRTd` to send a transaction then quit, this can also
be used by an ISP to link a user to a transaction.

#### Mitigation

Run `BRTd` as often as possible to conceal when transactions are being sent.
...
`BRTd` currently selects two outgoing connections every 5 minutes for
...
`BRTd` will have rotated its selected outgoing connections several times in
...
circuits is observable by the ISP. Instead, `BRTd` should likely disconnect
