...
# a main offline BRTd, does most of the tests
# a restricted RPC BRTd setup with RPC payment
# two local online BRTds connected to each other
N_BRTDS = 4

# 4 wallets connected to the main offline BRTd
# a wallet connected to the first local online BRTd
N_WALLETS = 5

WALLET_DIRECTORY = builddir + "/functional-tests-directory"
DIFFICULTY = 10

BRTd_base = [builddir + "/bin/BRTd", "--regtest", "--fixed-difficulty", str(DIFFICULTY), "--no-igd", "--p2p-bind-port", "BRTd_p2p_port", "--rpc-bind-port", "BRTd_rpc_port", "--zmq-rpc-bind-port", "BRTd_zmq_port", "--non-interactive", "--disable-dns-checkpoints", "--check-updates", "disabled", "--rpc-ssl", "disabled", "--data-dir", "BRTd_data_dir", "--log-level", "1"]
BRTd_extra = [
  ["--offline"],
...
wallet_base = [builddir + "/bin/BRT-wallet-rpc", "--wallet-dir", WALLET_DIRECTORY, "--rpc-bind-port", "wallet_port", "--disable-rpc-login", "--rpc-ssl", "disabled", "--daemon-ssl", "disabled", "--log-level", "1"]
wallet_extra = [
  ["--daemon-port", "18180"],
  ["--daemon-port", "18180"],
  ["--daemon-port", "18180"],
...
for i in range(N_BRTDS):
  command_lines.append([str(18180+i) if x == "BRTd_rpc_port" else str(18280+i) if x == "BRTd_p2p_port" else str(18380+i) if x == "BRTd_zmq_port" else builddir + "/functional-tests-directory/BRTd" + str(i) if x == "BRTd_data_dir" else x for x in BRTd_base])
  if i < len(BRTd_extra):
    command_lines[-1] += BRTd_extra[i]
  outputs.append(open(builddir + '/tests/functional_tests/BRTd' + str(i) + '.log', 'a+'))
