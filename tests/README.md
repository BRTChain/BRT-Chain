...
Core tests take longer than any other BRT tests, due to the high amount of computational work involved in validating core components.
...
To run only BRT's core tests (after building):
...
To run only BRT's crypto tests (after building):
...
```bash
BRTd --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `BRT-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only BRT's hash tests (after building):
...
To run only BRT's performance tests (after building):
...
To run only BRT's unit tests (after building):
